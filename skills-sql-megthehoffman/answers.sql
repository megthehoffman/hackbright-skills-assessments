...
SELECT id
FROM orders
WHERE customer_id = (SELECT id FROM customers WHERE fname = 'Jane' AND lname = 'Doe');
...
JOIN orders
ON customers.id = orders.customer_id
WHERE orders.processed = 't'
GROUP BY customers.fname, customers.lname, customers.email, orders.cupcake_id
HAVING orders.cupcake_id = 5;